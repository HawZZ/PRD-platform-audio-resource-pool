...
1. 作为 IoT 平台管理员，我想在配置中心统一新增和导入音源，以便多个产品或类别复用同一音源 id。
...
- `配置中心 > 音源管理` 菜单入口。
- 音源库：查询、新增、导入、删除、标签管理、批量配置标签、批量配置归属类别。
...
| FR-4 | 支持导入音源；平台提供导入模板下载和导入文件上传，不通过文本框手工输入 CSV 内容。 |
| FR-4A | 导入模板中的音源 id 必须按文本处理，导入模板表头使用中文展示。 |
| FR-4B | 导入时必须校验必填项：音源 ID、音源类型；在线音源必须填写在线资源 URL。 |
| FR-4C | 导入时必须校验音源 ID 是否重复，包含与现有音源重复和导入文件内重复。 |
| FR-4D | 导入时必须校验音源类型是否正确，仅支持本地 / 在线。 |
...
| FR-8 | 新增 / 导入音源时可设置一个或多个音源归属类别，选填。 |
| FR-9 | 新增 / 导入音源时可设置音源标签，选填。 |
...
- `配置中心` 下存在“音源管理”入口。
- 音源库支持新增 / 导入音源，并能查询音源 ID、归属类别和标签。
- 音源标签筛选支持筛选无标签音源。
- 导入音源提供模板下载和文件上传入口，不要求用户在页面中手工输入 CSV 文本；模板表头使用中文。
- 导入音源校验必填项、重复 ID、类型、类别、标签；导入结果展示总条数 / 成功数 / 失败数、失败原因，并支持下载失败列表。
...
| TC-4 导入音源模板 | 进入音源库 | 点击导入音源并下载模板 | 模板表头为中文；展示导入模板下载和导入文件上传，不展示 CSV 文本输入框 |
| TC-5 导入音源成功 | 导入文件中的 id 不重复，类型 / 类别 / 标签均有效 | 上传导入文件 | 结果展示总条数、成功数、失败数；成功项进入音源库 |
| TC-6 导入音源失败 | 导入文件存在重复 id、错误类型、错误类别或错误标签 | 上传导入文件 | 失败列表展示失败原因，并支持下载 |
